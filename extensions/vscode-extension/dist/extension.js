...
var CLIENT = "omp-vscode/0.2.0";
var MD_VIEW = "ompMarkdownAnnotator.preview";
var IMG_VIEW = "ompMarkdownAnnotator.imagePreview";
var PUML_VIEW = "ompMarkdownAnnotator.pumlPreview";
var RE_MD = /\.(?:md|markdown|mdown|mkd)$/i;
var RE_IMG = /\.(?:png|jpe?g|gif|webp|bmp)$/i;
var RE_PUML = /\.(?:puml|plantuml|iuml|wsd)$/i;
var markdownRenderer = new MarkdownIt({ html: false, linkify: true, typographer: true });
...
var Bridge = class {
  async send(uri, annotations, message, subject) {
    ...
    try {
      const cfgResponse = await fetch(`${base}/__bridge/config`, { signal: AbortSignal.timeout(2e3) });
...
function buildHtml({ webview, extensionUri, nonce, cspExtras, scripts, styles = [], initialPayload, bodyMarkup }) {
  ...
  const styleLinks = styles.map((rel) => `<link rel="stylesheet" href="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, "media", rel))}">`).join("\n  ");
  const scriptTags = scripts.map((rel) => `<script nonce="${nonce}" src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, "media", rel))}"></script>`).join("\n  ");
  const styleSrc = cspExtras?.styleUnsafeInline ? `${webview.cspSource} 'unsafe-inline'` : webview.cspSource;
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
var SIDEBAR_MARKUP = `<button id="badge" type="button" title="Arm annotator"><span>\u270E</span><strong>0</strong></button>
  <aside id="sidebar" aria-label="Annotations">
    <header><div><b>OMP Annotations</b><small id="document-title"></small></div><button id="close" type="button">\xD7</button></header>
